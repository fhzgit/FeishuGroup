...
_ATX_HEADING_PATTERN = re.compile(r"^\s{0,3}#{1,6}\s+(.+?)\s*$")
_SETEXT_HEADING_UNDERLINE_PATTERN = re.compile(r"^\s*(=+|-+)\s*$")
_MARKDOWN_IMAGE_PATTERN = re.compile(r"!\[([^\]]*)\]\((https?://[^)\s]+)\)")


def _flatten_markdown_headings(text: str) -> str:
    """
    将 Markdown 标题统一降为普通正文行（去掉 # / ==== 标题语法），避免飞书卡片中标题字号过大。
    支持:
    - ATX 标题: # / ## / ... / ######
    - Setext 标题: 标题行 + ==== / ----
    """
    ...
    while idx < len(lines):
        ...
        if not in_code_block:
            # ATX 标题（# 开头）-> 去掉标题标记，保留原文内联格式
            atx_match = _ATX_HEADING_PATTERN.match(line)
            if atx_match:
                title = atx_match.group(1).strip().rstrip("#").strip()
                result.append(title)
                idx += 1
                continue

            # Setext 标题（下一行为 ==== 或 ----）-> 仅保留标题正文
            if (
                idx + 1 < len(lines)
                and stripped
                and _SETEXT_HEADING_UNDERLINE_PATTERN.match(lines[idx + 1])
            ):
...
def _sanitize_ai_markdown(text: str) -> str:
    ...
    # 标题降级处理：避免飞书卡片渲染标题字号过大
    sanitized = _flatten_markdown_headings(sanitized)
    
    return sanitized


def _build_ai_markdown_elements(ai_answer: str) -> list[dict]:
    """
    将 AI Markdown 转换为卡片元素：
    - 普通文本保留为 markdown
    - 图片语法 ![alt](url) 转为“简化文案链接”（不展示完整 URL）
    """
    if not ai_answer:
    ...
    for match in _MARKDOWN_IMAGE_PATTERN.finditer(ai_answer):
        start, end = match.span()
        image_idx += 1
        alt_text = (match.group(1) or "").strip() or f"图片{image_idx}"
        image_url = match.group(2)

        # 图片前的 markdown 文本
        prefix = ai_answer[last_end:start]
        if prefix:
            elements.append({"tag": "markdown", "content": prefix})

        # 用“简化文案链接”替代原始图片展示，避免卡片 image_key 校验失败
        elements.append({
            "tag": "markdown",
            "content": f"🖼️ **图片{image_idx}**：[{alt_text}]({image_url})",
        })

        last_end = end

    tail = ai_answer[last_end:]
    if tail:
        elements.append({"tag": "markdown", "content": tail})

    if not elements:
        elements.append({"tag": "markdown", "content": ai_answer})
    return elements
...
def build_ai_reply_card(
    ...
    ai_elements = _build_ai_markdown_elements(ai_answer)
