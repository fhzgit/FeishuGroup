...
def _parse_uploaded_file_id(resp_data: dict) -> str:
    """从上传响应中尽量提取 file_id/token（不同版本字段命名可能不同）"""
    ...
    candidates = [
        ...
        data.get("token"),
        data.get("id"),
        (data.get("file_info") or {}).get("id") if isinstance(data.get("file_info"), dict) else "",
        (data.get("file_info") or {}).get("token") if isinstance(data.get("file_info"), dict) else "",
        (data.get("file") or {}).get("id") if isinstance(data.get("file"), dict) else "",
        (data.get("file") or {}).get("token") if isinstance(data.get("file"), dict) else "",
    ]
